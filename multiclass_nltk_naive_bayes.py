...
#divide dataset
index = int(len(labels_cc)/5)*4

X_train = abstracts_cc[:index]
y_train = labels_cc[:index]
pmids_train = pmids_dataset_cc[:index]

X_test = abstracts_cc[index:]
y_test = labels_cc[index:]
pmids_test = pmids_dataset_cc[index:]


#format the train set for NLTK
trainset = list()

for i in range(len(y_train)):
	trainset.append((word_feats(X_train[i]), y_train[i]))
