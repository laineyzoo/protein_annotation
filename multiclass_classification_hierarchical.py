...
def train_and_test(X_train, y_train, X_test, y_test):
	...
	#ada_real = AdaBoostClassifier(base_estimator=base,learning_rate=1,n_estimators=400,algorithm="SAMME")
...
def k_fold_validation(K, labels, abstracts, pmids_dataset):
    ...
    print("Multinomial NB: ", avg_acc[2])
    print("SVM: ", avg_acc[3])
    
    print("\nAverage F1-score:\n")
    print("Random Forest: ", avg_f1[0])
    print("AdaBoost (w/ SVM): ", avg_f1[1])
    print("Multinomial NB: ", avg_f1[2])
    print("SVM: ", avg_f1[3])
...
primary_labels = list()
primary_abstracts = list()
primary_pmids_dataset = list() #every datapoint will have an associated pubmed id in this list


cytoplasm_labels = list()
cytoplasm_abstracts = list()
cytoplasm_pmids_dataset = list() #every datapoint will have an associated pubmed id in this list
    ...
    for term in go_terms:
        ...
        for i in range(len(primary_components)):
            if term in primary_descendant_dict[primary_components[i]]:
                ...
                primary_abstracts.append(text)
                primary_pmids_dataset.append(pmid)
			...
			if primary_components[i] == "CYTOPLASM":
                for i in range(len(cytoplasm_components)):
					if term in cytoplasm_descendant_dict[cytoplasm_components[i]]:
						...
						cytoplasm_abstracts.append(text)
						cytoplasm_pmids_dataset.append(pmid)



#shuffle datasets
(primary_labels, primary_abstracts, primary_pmids_dataset) = shuffle_data(primary_labels, primary_abstracts, primary_pmids_dataset)
(cytoplasm_labels, cytoplasm_abstracts, cytoplasm_pmids_dataset) = shuffle_data(cytoplasm_labels, cytoplasm_abstracts, cytoplasm_pmids_dataset)

#k-fold validation
K=5 #K-fold validation
k_fold_validation(K, primary_labels, primary_abstracts, primary_pmids_dataset)
