...
def compute_softmax_scores(parent, test_point):
	...
	probabilities = np.empty(shape=(len(classifiers), 1))
	for child in direct_children:
		y_train = list()
		for i in range(len(pmids_train)):
			matching = data_cc[data_cc[:,2]==pmids_train[i]]
			go_terms = list(set(matching[:,1]))
		...
		np.append(classifier.predict_proba(test_point)[0,1], probabilities)
	softmax_scores = softmax(probabilities)
	return softmax_scores


def traverse_ontology(parent, test_point, ontology_scores):
	children = get_direct_descendants(parent)
	if len(children)>0:
		scores = compute_softmax_scores(parent, test_point)
		ontology_scores[parent] = scores
		for child in children:
			traverse_ontology(child, test_point, ontology_scores)
...
#for each test point, traverse the ontology from root to leaf
for test_point in X_test:
	root = GO_ONTOLOGIES["CELLULAR_COMPONENT"]
	ontology_scores = {}
	traverse_ontology(root, test_point, ontology_scores)
